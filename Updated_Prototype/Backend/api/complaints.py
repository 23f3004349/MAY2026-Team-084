...
def get_complaints():
    ...
    if user.role in ["ADMIN", "TREASURER", "COMMITTEE_MEMBER", "SYSTEM_ADMIN"]:
        complaints = Complaint.query.order_by(Complaint.created_at.desc()).all()
    elif user.role == "WORKER":
        complaints = Complaint.query.filter(
            (Complaint.raised_by == user_id) | (Complaint.assigned_worker_id == user_id)
...
def update_status(cid):
    ...
    if not user or user.role not in ["ADMIN", "SYSTEM_ADMIN", "TREASURER", "COMMITTEE_MEMBER", "WORKER"]:
        return jsonify({"error": "Not permitted to update complaint status"}), 403
