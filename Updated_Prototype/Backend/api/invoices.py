...
def mark_paid(inv_id):
    ...
    is_admin = user and user.role in ["ADMIN", "TREASURER"]
    is_owner = resident and resident.user_id == user_id
    if not (is_admin or is_owner):
        return jsonify({"error": "You can only pay your own invoice"}), 403
    if not resident:
    ...
    payment = Payment(
        ...
        payment_method=data.get("payment_method") or data.get("mode", "CASH"),
