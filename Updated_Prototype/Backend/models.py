...
# ----------------------------------------------------------------
# FEATURE MODELS FOR THE NEW SOCIETY EXPERIENCE
# ----------------------------------------------------------------
...
class VisitorPass(db.Model):
    ...
    pass_type = db.Column(db.String(40), default="Visitor")
    slot = db.Column(db.String(40), nullable=False)
    status = db.Column(db.String(40), default="Pending")
    eta = db.Column(db.String(80))
    pass_code = db.Column(db.String(80), unique=True, nullable=False)
...
class AmenityBooking(db.Model):
    __tablename__ = "amenity_bookings"

    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey("users.id"), nullable=False)
    amenity_name = db.Column(db.String(100), nullable=False)
    slot_time = db.Column(db.String(40), nullable=False)
    recurring = db.Column(db.Boolean, default=False)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)

    __table_args__ = (
        db.UniqueConstraint("user_id", "amenity_name", "slot_time", name="uq_amenity_booking_user_slot"),
    )
...
class EventAttendance(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint("event_id", "user_id", name="uq_event_attendance_user"),
    )
...
class MarketplaceListing(db.Model):
    ...
    category = db.Column(db.String(60), default="For Sale")
    condition = db.Column(db.String(60), default="New listing")
    seller_name = db.Column(db.String(100), nullable=False)
    seller_flat = db.Column(db.String(40))
...
class MarketplaceMessage(db.Model):
    ...
    listing = db.relationship("MarketplaceListing", back_populates="messages")

with app.app_context():
    db.create_all()

    def seed_user(email, password, role, name):
    ...
    admin = seed_user("admin@apt.com", "Admin@123", "ADMIN", "Administrator")
    owner = seed_user("owner@apt.com", "Owner@123", "OWNER", "Asha Nair")
    worker = seed_user("worker@apt.com", "Worker@123", "WORKER", "Ravi Kumar")

    apt_a101 = Apartment.query.filter_by(flat_number="A-101").first()
    if not apt_a101:
        apt_a101 = Apartment(flat_number="A-101", block="A", floor=1)
        db.session.add(apt_a101)
        db.session.commit()

    apt_b204 = Apartment.query.filter_by(flat_number="B-204").first()
    if not apt_b204:
        apt_b204 = Apartment(flat_number="B-204", block="B", floor=2)
        db.session.add(apt_b204)
        db.session.commit()

    resident = Resident.query.filter_by(user_id=owner.id).first()
    if not resident:
        resident = Resident(user_id=owner.id, apartment_id=apt_a101.id, is_owner=True, move_in_date=date(2024, 1, 1))
        db.session.add(resident)
        db.session.commit()

    if not Invoice.query.filter_by(apartment_id=apt_a101.id, month=datetime.utcnow().month, year=datetime.utcnow().year).first():
        invoice = Invoice(
            apartment_id=apt_a101.id,
    ...
    if not Complaint.query.first():
        complaint = Complaint(
            ...
            description="The corridor light keeps tripping after 9 pm. Please check the switchboard.",
            category="ELECTRICAL",
            priority="HIGH",
            status="OPEN",
        )
    ...
    if not Announcement.query.first():
        notice = Announcement(
            ...
            content="The water tank will be cleaned on Saturday between 10 am and 1 pm. Please store water in advance.",
    ...
    existing_poll = Vote.query.first()
    if not existing_poll:
        poll = Vote(
            title="Install EV charging points in basement",
            description="Do you support the EV charging pilot for residents?",
            created_by=admin.id,
        ...
        db.session.flush()
        db.session.add_all([
            VoteOption(vote_id=poll.id, option_text="Yes, install them"),
            VoteOption(vote_id=poll.id, option_text="No, keep current setup"),
        ])
        db.session.commit()
    else:
        existing_texts = {opt.option_text for opt in existing_poll.options}
        for option_text in ["Yes, install them", "No, keep current setup"]:
            if option_text not in existing_texts:
                db.session.add(VoteOption(vote_id=existing_poll.id, option_text=option_text))
        db.session.commit()

    if not MaintenanceTask.query.first():
        task = MaintenanceTask(
            title="Lift inspection",
            description="Routine safety inspection for the main lift.",
            category="ELECTRICAL",
    ...
    print("Database created with demo data!")